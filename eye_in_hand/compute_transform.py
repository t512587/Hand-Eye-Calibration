...
# === 採集數據 JSON 檔案路徑 ===
CALIBRATION_DATA_JSON_PATH = r"C:\Users\user\Downloads\robottest-main\c2h_trans\eye_on_hand\handeye_records\handeye_chessboard_20260119_132556.json"
...
def calculate_handeye_calibration(json_path):
    ...
    data = []
    for d in raw_data:
        tvec_key = "aruco_tvec" if "aruco_tvec" in d else "chessboard_tvec"
        rvec_key = "aruco_rvec" if "aruco_rvec" in d else "chessboard_rvec"
        
        aruco_tvec = [x / 1000 for x in d[tvec_key]]  # mm to m
        robot_pose = d["robot_pose_at_detect"].copy()
        robot_pose[:3] = [x / 1000 for x in robot_pose[:3]]  # mm to m
        
        data.append({
            "aruco_tvec": aruco_tvec,
            "aruco_rvec": d[rvec_key],
